...
STATIC void   hali2cSend( uint8_t *buffer, uint16_t len, uint8_t sendStart, uint8_t sendStop );
STATIC _Bool  hali2cSendByte( uint8_t dByte );
STATIC void   hali2cWrite( int8_t dBit );
STATIC void   hali2cClock( int8_t dir );
STATIC void   hali2cStart( void );
STATIC void   hali2cStop( void );
STATIC void   hali2cReceive( uint8_t address, uint8_t reg, uint8_t *buffer, uint16_t len );
STATIC uint8_t  hali2cReceiveByte( void );
STATIC _Bool  hali2cRead( void );
STATIC void   hali2cSendDeviceAddress(uint8_t address);


STATIC void   hali2cWait( uint8_t );

STATIC uint8_t s_xmemIsInit;
...
int8_t HalI2CReceive(uint8_t address, uint8_t reg, uint8_t *buf, uint16_t len)
{
  hali2cReceive(address, reg, buf, len);
...
int8_t HalI2CSend(uint8_t address, uint8_t *buf, uint16_t len)
{
  ...
  hali2cSendDeviceAddress((address<<1)&0xFE);
...
{
  uint16_t i;
  uint8_t retry = HAL_I2C_RETRY_CNT;
 ...
 * @return  void
 */
STATIC void hali2cWait( uint8_t count )
{
  while ( count-- );
}
 ...
 */
STATIC uint8_t hali2cReceiveByte()
{
  int8_t i, rval = 0;
...
STATIC void hali2cReceive( uint8_t address, uint8_t reg, uint8_t *buffer, uint16_t len )
{
  ...
  hali2cSendDeviceAddress((address<<1)&0xFE);
  hali2cSend(&reg, 1, NOSEND_START, NOSEND_STOP);
  hali2cSendDeviceAddress((address<<1)|0x01);
  //hali2cSend((address<<1)|0x01,1,SEND_START, NOSEND_STOP);
